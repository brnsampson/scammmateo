...
func (s *Server) ServeWithReload(router http.Handler, sopts ServerOptions, reload func() error) {
	for {
        if err := reload(); err != nil {
			s.logger.Error("Error: failed to refresh server config. May use some old settings.")
        }

		addr, err := sopts.GetAddr()
		if err != nil {
			s.logger.Error("Error: failed to get address from server config! Cannot start server!")
			s.err <- err
		}
		tlsConf := sopts.GetTlsConfig()
		tlsEnabled := sopts.GetTlsEnabled()
...
func (s *Server) Run(router http.Handler, sopts ServerOptions, reload func() error) {
	s.wg.Add(1)
	go s.waitOnInterrupt()
	go s.ServeWithReload(router, sopts, reload)
	return
}

func (s *Server) BlockingRun(router http.Handler, opts ServerOptions, reload func() error) int {
	s.wg.Add(1)
	go s.ServeWithReload(router, opts, reload)
	s.waitOnInterrupt()
	return s.exitCode
}
